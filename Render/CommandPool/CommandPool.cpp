...
void CommandPool::Initialization(vkb::Device device, vkb::QueueType queueType)
{
	...
	DEV_ASSERT(vkCreateCommandPool(device.device, &commandPoolCreateInfo, nullptr, &commandPool), "CommandPool", "Error creating the Command Pool!");
	DEV_LOG(TE_INFO, "CommandPool", "CommandPool %s created!", queueType);
}
